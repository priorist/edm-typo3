...
class EventController extends AbstractController
{
	...
	protected function abstractDetailAction(bool $showAll)
	{
		...
		if (isset($_GET['id']) && $this->request->hasArgument('eventBaseSlug')) {
	...
	protected function getEventsBasedOnId(array $settings, bool $showAll)
	{
		...
		if ($eventBase !== NULL) {
			...
			try {
				$event = $this->getClient()->getRestClient()->event->findById($eventId, $eventParams);
			} catch (\Throwable $e) {
				$this->view->assign('internalError', true);
				return;
			}

			$eventCities = $this->getLocationCities($event, true);
			if ($showAll === true) {
				$sanitizedEvent = $event;
			} else {
				$sanitizedEvent = $this->sanitizeEvents($event);
			}

			$sanitizedEvent = $this->prepareEventPriceData($sanitizedEvent);

			if (!isset($sanitizedEvent)) {
				$this->view->assign('noEventAvailable', true);
			}

			$this->view->assign('cities', $eventCities);
			$this->view->assign('events', $sanitizedEvent);
